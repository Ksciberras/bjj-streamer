...
function AppShell({ user, active, canUpload, onNavigate, onLogout, children }: { user: User; active: View | 'study'; canUpload: boolean; onNavigate: (view: View) => void; onLogout: () => Promise<void>; children: ReactNode }) {
  const navigation: { id: View; label: string; show: boolean }[] = [
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      <Wordmark detail />
      <nav aria-label="Primary navigation">{navigation.filter((item) => item.show).map((item) => <button key={item.id} className={active === item.id ? 'nav-link active' : 'nav-link'} onClick={() => onNavigate(item.id)} aria-current={active === item.id ? 'page' : undefined}>{item.label}</button>)}</nav>
      <div className="account"><div className="account-avatar" aria-hidden="true">{user.email.charAt(0).toUpperCase()}</div><div className="account-copy"><strong>{user.email}</strong><span>{user.role}</span></div><button className="account-action" onClick={() => void onLogout()}>Sign out</button></div>
    </aside>
    <header className="mobile-header"><Wordmark /><details className="account-menu"><summary aria-label="Open account menu"><span className="account-avatar" aria-hidden="true">{user.email.charAt(0).toUpperCase()}</span></summary><div>{user.role === 'admin' && <button onClick={() => onNavigate('admin')}>Admin</button>}<button onClick={() => void onLogout()}>Sign out</button></div></details></header>
    <main className="content" id="main-content">{children}</main>
    <nav className="mobile-nav" aria-label="Mobile navigation">{navigation.filter((item) => item.show && item.id !== 'admin').map((item) => <button key={item.id} className={active === item.id ? 'active' : ''} onClick={() => onNavigate(item.id)} aria-current={active === item.id ? 'page' : undefined}>{item.label}</button>)}</nav>
  </div>
}
...
function HomeScreen({ videos, progress, loading, openVideo, browse }: { videos: Video[]; progress: ProgressMap; loading: boolean; openVideo: (video: Video) => void; browse: (filter: { instructor?: string; tag?: string }) => void }) {
  ...
  return <div className="screen home-screen"><PageHeader title="Home" description="Pick up where you left off." />
...
function LibraryScreen({ videos, progress, loading, initialFilter, openVideo, onSearch }: { videos: Video[]; progress: ProgressMap; loading: boolean; initialFilter: { instructor?: string; tag?: string }; openVideo: (video: Video) => void; onSearch: (query: string) => Promise<void> }) {
  ...
  return <div className="screen"><PageHeader title="Library" description={`${filtered.length} accessible ${filtered.length === 1 ? 'video' : 'videos'}`} />
...
function VideoCard({ video, savedAt = 0, onOpen }: { video: Video; savedAt?: number; onOpen: () => void }) {
  return <article className="video-card"><button className="video-cover" onClick={onOpen} aria-label={`Study ${video.title}`}><span className="video-placeholder" aria-hidden="true"><strong>{initials(video.instructor_name)}</strong><small>Study video</small></span>{savedAt > 0 && <span className="resume-chip">{formatTime(savedAt)} saved</span>}</button><div className="video-card-body"><div className="video-title-row"><h2>{video.title}</h2><Visibility value={video.visibility} /></div><p>{video.instructor_name}</p>{(video.instructional_name || video.chapter_name) && <small>{[video.instructional_name, video.chapter_name].filter(Boolean).join(' · ')}</small>}<button className="card-action" onClick={onOpen}>{savedAt > 0 ? 'Resume' : 'Study video'} <span aria-hidden="true">→</span></button></div></article>
}

function ContinueCard({ video, savedAt, onResume }: { video: Video; savedAt: number; onResume: () => void }) {
  return <article className="continue-card"><button className="continue-cover" onClick={onResume} aria-label={`Resume ${video.title}`}><span className="video-placeholder" aria-hidden="true"><strong>{initials(video.instructor_name)}</strong><small>Continue watching</small></span></button><div className="continue-copy"><Visibility value={video.visibility} /><h2>{video.title}</h2><p>{video.instructor_name}{video.instructional_name ? ` · ${video.instructional_name}` : ''}{video.chapter_name ? ` · ${video.chapter_name}` : ''}</p><div className="saved-position"><span>Saved position</span><strong>{formatTime(savedAt)}</strong></div><button onClick={onResume}>Resume at {formatTime(savedAt)} <span aria-hidden="true">→</span></button></div></article>
}
...
function StudyScreen({ video, onBack, setError, onProgress }: { video: Video; onBack: () => void; setError: (message: string) => void; onProgress: (seconds: number) => void }) {
  ...
  useEffect(() => {
    const keys = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Escape') {
        ...
        if (editor) { event.preventDefault(); editor.querySelector<HTMLButtonElement>('[data-cancel-edit]')?.click() }
        return
      }
  ...
  return <div className="screen study-screen"><button className="back-button" onClick={() => void back()}>← Back to library</button><div className="study-layout"><section className="player-column" aria-labelledby="video-title"><div className="player-frame">{loading ? <div className="player-loading">Loading video…</div> : url ? <video ref={player} src={url} controls playsInline preload="metadata" onLoadedMetadata={() => { if (player.current && resumeAt > 0) player.current.currentTime = resumeAt }} onTimeUpdate={timeUpdate} onPause={() => void saveProgress()} /> : <ErrorState title="Couldn’t load this video" body="Check your connection and try again." />}</div><div className="player-details"><div><div className="detail-line"><Visibility value={video.visibility} /><span>{video.instructional_name || 'Instructional video'}</span></div><h1 id="video-title">{video.title}</h1><p>{video.instructor_name}{video.chapter_name ? ` · ${video.chapter_name}` : ''}</p>{resumeAt > 0 && <p className="resume-detail">Resumed from <code>{formatTime(resumeAt)}</code></p>}</div>{video.tags.length > 0 && <div className="tag-list compact">{video.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>}</div></section><NotesPanel notes={sortedNotes} draftTimestamp={draftTimestamp} noteInput={noteInput} onCapture={captureNoteTime} onAdd={addNote} onSeek={seek} onUpdate={updateNote} onDelete={deleteNote} /></div></div>
}
...
function NotesPanel({ notes, draftTimestamp, noteInput, onCapture, onAdd, onSeek, onUpdate, onDelete }: { notes: Note[]; draftTimestamp: number; noteInput: React.RefObject<HTMLTextAreaElement | null>; onCapture: () => void; onAdd: (event: FormEvent<HTMLFormElement>) => void; onSeek: (seconds: number) => void; onUpdate: (event: FormEvent<HTMLFormElement>, note: Note) => void; onDelete: (note: Note) => void }) {
  return <aside className="notes-panel" aria-labelledby="notes-title"><div className="notes-heading"><div><h2 id="notes-title">Notes</h2><p>Private to you</p></div><span aria-label={`${notes.length} notes`}>{notes.length}</span></div><button className="capture-button" onClick={onCapture}>Add note at {formatTime(draftTimestamp)} <kbd>N</kbd></button><form className="note-composer" onSubmit={onAdd}><label><span>Note at <code>{formatTime(draftTimestamp)}</code></span><textarea ref={noteInput} name="body" required maxLength={5000} placeholder="Add a detail to revisit…" /></label><button type="submit">Save note</button></form><div className="notes-list">{notes.length ? notes.map((note) => <NoteItem key={note.id} note={note} onSeek={onSeek} onUpdate={onUpdate} onDelete={onDelete} />) : <EmptyState title="No notes yet" body="Add a note at the current timestamp." />}</div></aside>
}

function NoteItem({ note, onSeek, onUpdate, onDelete }: { note: Note; onSeek: (seconds: number) => void; onUpdate: (event: FormEvent<HTMLFormElement>, note: Note) => void; onDelete: (note: Note) => void }) {
  const [editing, setEditing] = useState(false)
  function submit(event: FormEvent<HTMLFormElement>) { onUpdate(event, note); setEditing(false) }
  return <article className="note-item" data-editing={editing}>{editing ? <form onSubmit={submit}><label><span className="sr-only">Timestamp</span><input name="timestamp_seconds" type="number" min="0" step="0.1" defaultValue={note.timestamp_seconds} /></label><label><span className="sr-only">Note</span><textarea name="body" defaultValue={note.body} required maxLength={5000} autoFocus /></label><div className="note-actions"><button type="submit">Save</button><button type="button" className="secondary-button" data-cancel-edit onClick={() => setEditing(false)}>Cancel</button><button type="button" className="danger-button" onClick={() => onDelete(note)}>Delete</button></div></form> : <div className="note-reading"><button type="button" className="timestamp" onClick={() => onSeek(note.timestamp_seconds)}>{formatTime(note.timestamp_seconds)}</button><p>{note.body}</p><button type="button" className="edit-note" onClick={() => setEditing(true)}>Edit</button></div>}</article>
}
...
function UploadScreen({ user, videos, onUploaded, onError, onUpdate }: { user: User; videos: Video[]; onUploaded: () => Promise<void>; onError: (value: string) => void; onUpdate: () => Promise<void> }) {
  ...
  return <div className="screen"><PageHeader title="Upload MP4" description="Add one browser-compatible MP4 to the library." /><div className="upload-layout"><section className="surface upload-panel"><form className="upload-form" onSubmit={upload}><div className="form-step"><span>1</span><label>Select MP4<input name="file" type="file" accept="video/mp4,.mp4" required onChange={(event) => setFile(event.target.files?.[0] ?? null)} /></label>{file && <div className="file-summary"><strong>{file.name}</strong><span>{formatBytes(file.size)} · MP4</span></div>}</div><div className="form-step"><span>2</span><div className="field-grid"><label>Title<input name="title" required maxLength={200} /></label><label>Instructor<input name="instructor_name" required maxLength={200} /></label><label>Instructional / series<input name="instructional_name" maxLength={200} /></label><label>Chapter<input name="chapter_name" maxLength={200} /></label><label className="full">Description<textarea name="description" maxLength={10000} /></label><label className="full">Tags, comma separated<input name="tags" /></label></div></div><div className="form-step"><span>3</span><div className="field-grid"><label>Visibility<select name="visibility"><option value="shared">Shared with members</option><option value="private">Private video</option></select></label><label>Content basis<select name="content_basis"><option value="self_created">Self-created</option><option value="licensed_for_group">Licensed for group</option><option value="personal_purchase">Personal purchase (private only)</option></select></label></div></div><div className="upload-submit"><button type="submit" disabled={state === 'uploading'}>{state === 'uploading' ? `Uploading ${progress ?? 0}%` : state === 'error' ? 'Try upload again' : 'Upload MP4'}</button><p>Use a browser-compatible MP4 up to 5 GiB. Uploads are not resumable, so keep this page open.</p>{progress !== null && <div className="progress-track" role="progressbar" aria-label="Upload progress" aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress}><span style={{ width: `${progress}%` }} /></div>}{state === 'success' && <p className="success-text" role="status">Upload complete. The video is ready.</p>}</div></form></section><ManageVideos videos={manageable} onUpdate={onUpdate} onError={onError} /></div></div>
}
...
function Visibility({ value }: { value: Video['visibility'] }) { return <span className={`visibility ${value}`}>{value === 'private' ? 'Private video' : 'Shared with members'}</span> }
...
function EmptyState({ title, body, action }: { title: string; body: string; action?: ReactNode }) { return <div className="empty-state"><h3>{title}</h3><p>{body}</p>{action}</div> }
