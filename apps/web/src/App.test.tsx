...
describe('App', () => {
  ...
  it('loads account management for an administrator', async () => {
	...
	fireEvent.click(screen.getAllByRole('button', { name: 'Content' })[0])
  ...
  it('shows upload but not administration to an instructor', async () => {
    ...
    expect(screen.getByRole('heading', { name: 'Build a course' })).toBeInTheDocument()
  ...
  it('shows gym and availability controls only to the platform owner', async () => {
    ...
    fireEvent.click(screen.getByRole('tab', { name: /^Gyms/ }))
    ...
    fireEvent.click(screen.getByRole('tab', { name: 'Content access' }))
